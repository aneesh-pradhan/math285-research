...
\begin{equation}
  ...
  \qquad \vC(0) = 0\,\mathrm{V},\quad \dot{\vC}(0) = 0.
  \label{eq:rlc_ivp}
\end{equation}
...
\begin{itemize}
  \item \textbf{Independent variable:} time $t$ in $\mathrm{s}$.
  \item \textbf{Dependent variables:} $\vC(t)$ in $\mathrm{V}$ (capacitor
        voltage) and $i(t)$ in $\mathrm{A}$ (series current).
\end{itemize}
...
\begin{itemize}
  ...
  \item $L = 0.1\,\mathrm{H}$: inductance; stores energy in a magnetic field.
        Governs how quickly current can change.
  \item $C = 10\times 10^{-6}\,\mathrm{F}$: capacitance; stores energy in an
        electric field. Governs how quickly voltage can change.
  \item $\Vs = 10\,\mathrm{V}$: step-input amplitude. Represents a DC voltage
        switched on at $t = 0$.
\end{itemize}
...
Section~\ref{sec:nonideal} relaxes two of these assumptions by simulating
current-dependent resistor heating and inductor saturation.
...
\begin{itemize}
  ...
  \item \textbf{Critically damped} ($\Delta = 0$, i.e.\ $R = R_\mathrm{crit} = 2\sqrt{L/C}$):
...
from the ODE.
For example, in the underdamped case one may write
\begin{equation}
  \vC(t) = \Vs + u_1(t)e^{-\alpha t}\cos\omega_d t
    + u_2(t)e^{-\alpha t}\sin\omega_d t,
  \label{eq:vop_form}
\end{equation}
where $u_1(t)$ and $u_2(t)$ are chosen so that the forced equation and its
auxiliary constraint are both satisfied.
This converts a nonconstant forcing term into first-order equations for
$u_1$ and $u_2$~\cite{nilsson2019}.
Alternatively,
...
followed by a partial-fraction expansion and inverse transform~\cite{hayt2018}.

\paragraph{Applying initial conditions (underdamped case)}
Here $V_0$ denotes the general initial capacitor voltage; the main case
study in Table~\ref{tab:nominal-values} uses $V_0 = 0$, consistent with the
zero-charge initial condition in~\eqref{eq:rlc_ivp}.
...
\end{align}
Substituting these coefficient equations into \eqref{eq:under} and adding
$\Vs$ gives the complete analytic underdamped step response:
\begin{equation}
  \vC(t) = \Vs + (V_0 - \Vs)e^{-\alpha t}\left(
    \cos\omega_d t + \frac{\alpha}{\omega_d}\sin\omega_d t\right).
  \label{eq:underdamped_final}
\end{equation}
For the typical step-response case $V_0 = 0$, this simplifies to
$\vC(t) = \Vs\left[1 - e^{-\alpha t}\left(\cos\omega_d t + \frac{\alpha}{\omega_d}\sin\omega_d t\right)\right]$.

For completeness, the initial conditions $\vC(0) = 0$ and $\dot{\vC}(0) = 0$
are applied to the remaining two cases.
For the critically damped case, the complete solution is
$\vC(t) = v_{C,h}(t) + \Vs$ with
$v_{C,h}(t) = (A_1 + A_2 t)\,e^{-\alpha t}$.
Applying the initial conditions:
\begin{align}
  \vC(0) &= A_1 + \Vs = 0 \;\implies\; A_1 = -\Vs, \label{eq:crit_ic1}\\
  \dot{\vC}(0) &= -\alpha A_1 + A_2 = 0 \;\implies\;
                 A_2 = -\alpha\Vs, \label{eq:crit_ic2}
\end{align}
giving the complete critically damped step response:
\begin{equation}
  \vC(t) = \Vs\left[1 - (1 + \alpha t)\,e^{-\alpha t}\right].
  \label{eq:crit_final}
\end{equation}
...
Applying the initial conditions:
\begin{align}
  \vC(0) &= A_1 + A_2 + \Vs = 0 \;\implies\; A_1 + A_2 = -\Vs,
    \label{eq:over_ic1}\\
  \dot{\vC}(0) &= r_1 A_1 + r_2 A_2 = 0,
    \label{eq:over_ic2}
\end{align}
and solving this $2\times2$ system:
\begin{equation}
  A_1 = -\Vs\,\frac{r_2}{r_2 - r_1}, \qquad
  A_2 = \Vs\,\frac{r_1}{r_2 - r_1},
  \label{eq:over_coeffs}
\end{equation}
giving the complete overdamped step response:
\begin{equation}
  \vC(t) = \Vs\left[1 -
    \frac{r_2\,e^{r_1 t} - r_1\,e^{r_2 t}}{r_2 - r_1}\right].
  \label{eq:over_final}
\end{equation}
